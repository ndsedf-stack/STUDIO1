...
const muscleGroups = ["Pectoraux", "Dos", "Quadriceps", "Ischios", "Fessiers", "Épaules", "Biceps", "Triceps", "Avant-bras", "Mollets"];
...
const useWorkoutHistory = () => {
    ...
    const importHistory = useCallback((newHistory) => {
        try {
            // Basic validation
            if (typeof newHistory === 'object' && newHistory !== null && Object.keys(newHistory).length > 0) {
                 localStorage.setItem(DB_KEY, JSON.stringify(newHistory));
                 setHistory(newHistory);
                 alert('Historique importé avec succès ! L\'application va se recharger.');
...
const ActiveWorkoutView = ({ workout, meta, onEndWorkout, getSuggestedWeight }) => {
    ...
    const [workoutState, setWorkoutState] = useState(() => 
        workout.exercises.map((exo) => {
            if (exo.type === 'superset') {
              ...
              return { ...exo, exercises: exo.exercises.map(subExo => ({...subExo, sets: Array.from({length: numSets}, (_,i) => ({id:i, weight: getSuggestedWeight(subExo) || '', reps: (subExo.reps || "8").toString().split('-')[0], rir: subExo.rir || 1, completed: false})) })) };
            }
    ...
    const handleSetComplete = (isCompleted, setIndex, subExoIndex = -1) => {
        ...
        if (newExo.type === 'superset') {
            newExo.exercises.forEach((subExo, sIdx) => {
                if (subExo.sets[setIndex]) {
                    subExo.sets[setIndex].completed = isCompleted;
                }
            });
        } else {
            const set = newExo.sets[setIndex];
...
const DataManagementView = ({ history, onImport }) => {
    ...
    const handleExport = () => {
        ...
        link.download = 'hybrid_master_backup.json';
        link.href = url;
        link.click();
    ...
    return React.createElement("div", { className: "data-management-section" },
        React.createElement("button", { onClick: handleExport, className: "data-management-btn" }, "Exporter l'historique"),
        React.createElement("button", { onClick: handleImportClick, className: "data-management-btn import" }, "Importer l'historique"),
        React.createElement("input", { type: "file", ref: fileInputRef, onChange: handleFileChange, style: { display: 'none' }, accept: ".json" })
    );
};

const StatisticsView = ({ getExercisePR, history, onImport }) => {
    ...
    return (
      React.createElement("div", { className: "main-content" },
        ...
        React.createElement("div", { className: "stats-section" }, 
            React.createElement("h3", null, "💾 Gestion des Données"), 
            React.createElement(DataManagementView, { history: history, onImport: onImport })
        )
      )
    );
};
...
const App = () => {
  ...
  const handleEndWorkout = (completedWorkout) => {
    if (completedWorkout) saveWorkout({ date: new Date().toISOString(), ...activeWorkout.meta, exercises: completedWorkout });
    setActiveWorkout(null);
  };

  const renderContent = () => {
    if (activeWorkout) {
        return React.createElement("div", { className: "main-content" }, React.createElement(ActiveWorkoutView, { key: activeWorkout.meta.startTime, workout: activeWorkout.workout, meta: activeWorkout.meta, onEndWorkout: handleEndWorkout, getSuggestedWeight: getSuggestedWeight }));
    }
    switch (currentView) {
      case 'program':
        return React.createElement(WorkoutPlannerView, { onStartWorkout: handleStartWorkout });
      case 'stats':
        return React.createElement(StatisticsView, { getExercisePR: getExercisePR, history: history, onImport: importHistory });
      default:
        return null;
    }
  };
